...
import com.socyno.base.bscmodel.ObjectMap0;
...
public class SqlQueryUtil {
	...
	public static boolean checkSQLName(String name, boolean noException) {
		if (name == null || !REGEXP_SQL_NAME.matcher(name).find()) {
			...
			throw new MessageException(String.format("无效的数据库表或列名称(%s)", name));
		}
		return true;
	}
     ...
     * @return
     */
    public static AbstractSqlStatement pairs2InsertQuery(String table, @NonNull List<? extends ObjectMap0<?>> kvPairs, ObjectMap0<?> dupPairs) {
        String[] keys = null;
        ...
        return pairs2InsertQuery(table, Arrays.asList(keys),
                values, (dupPairs == null) ? null : dupPairs.asMap() );
    }
        ...
        for (Map.Entry<String, Object> e : kvPairs.entrySet()) {
            ...
            if (fieldAsWhere || SQLTYPE.DELETE.equals(type)) {
                ...
                if (valueAsPlain) {
                    ...
                    sqlWhere.append(StringUtils.join("?", ((String[])value).length,
                                        ",", " IN (", ")"));
