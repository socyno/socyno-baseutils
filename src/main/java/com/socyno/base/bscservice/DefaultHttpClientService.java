...
public class DefaultHttpClientService {
	...
	public static JsonElement getResponseJson(CloseableHttpResponse resp,
		...
		return JsonUtil.fromJson(getResponseText(resp, charset),
				JsonElement.class);
	}

	public static <T> T getResponseJson(CloseableHttpResponse resp,
			String charset, Class<T> clazz) throws IOException {
		return JsonUtil.fromJson(getResponseText(resp, charset), clazz);
	}
	...
	public CloseableHttpResponse postJson(String url, Object data)
		...
		byte[] body = null;
		if (data != null) {
			body = StringUtils.trimToEmpty(JsonUtil.toJson(data)).getBytes(
					"UTF-8");
		}
